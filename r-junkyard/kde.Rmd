...
The best way I know to density estimation in the R ecosystem is using the [`spatstat`] library's specialisation of base *R*'s `density` function. That means converting the point data to a `spatstat` planar point pattern (`ppp`) object
...
Here we specify the size of the raster we want with `dimyx` (note the order, y then x) using the number of pixels (`height` and `width`) we calculated earlier. We can convert this directly to a raster, but have to supply a CRS which we pull from the original points input dataset.

```{r}
kde <- density(pts.ppp, sigma = 200, dimyx = c(height, width)) %>%
  raster(crs = st_crs(pts)$wkt) # you have to do this because CRS-chaos in the FOSS-sphere
```
