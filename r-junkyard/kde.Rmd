...
date: "12 August, 2021"
---
[Download this notebook](kde.Rmd?raw=true).
...
Optionally web maps are nice:
```{r}
# tmap_mode("view")
```

## Data
And the data for this one are some [point data](abb.gpkg?raw=true) (Airbnb listings from [here](http://insideairbnb.com/new-zealand/)) and some [polygon data](sa2.gpkg?raw=true) (NZ census Statistical Area 2 data).
...
```{r}
tm_shape(polys) +
  tm_polygons(col = "n", palette = "viridis", title = "Points in polygons")
```

### `spatstat` for density estimation
The best way I know to do density estimation in the R ecosystem is using the [`spatstat`] library's specialisation of base *R*'s `density` function. That means converting the point data to a `spatstat` planar point pattern (`ppp`) object
...
Here we specify the size of the raster we want with `dimyx` (note the order, y then x) using the number of pixels (`height` and `width`) we calculated earlier. We can convert this directly to a raster, but have to supply a CRS which we pull from the original points input dataset. At the time of writing (August 2021) you'll get a complaint about the New Zealand Geodetic Datum 2000 because recent changes in how projections and datums are handled are still working themselves out.

```{r}
kde <- density(pts.ppp, sigma = 200, dimyx = c(height, width)) %>%
  raster(crs = st_crs(pts)$wkt) # a ppp has no CRS information
```

We can map this - the default colour ramp in `raster` is horrible, so usr `RColorBrewer::brewer.pal` to get something nicer.
...
```{r}
tm_shape(polys) + 
  tm_polygons(col = "estimated_count", palette = "viridis", title = "200m KDE estimate")
```

[Back to the junkyard](../r-junkyard.md)
