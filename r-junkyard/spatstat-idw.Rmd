...
---
Get this notebook [here](spatstat-idw.Rmd?raw=true).
...
We also need a spatial extent for the intepolation, so let's just make a convex hull of the points
...
A [previous notebook](kde.html) showed an even quicker way to do this, but where the window will be formed from a bounding box (and where's the fun in that?)
...
```{r}
result <- idw(pp, power = 0.5, eps = 10)
plot(result)
```
