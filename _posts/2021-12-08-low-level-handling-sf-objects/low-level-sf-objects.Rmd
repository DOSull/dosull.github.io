...
## Making polygons
My main confusion dealing with polygons in `sf` sounds dumb, but was easily fixed. Matrices in *R* get populated by column, by default, where we typically think of the points in a polygon as being specified as rows of x-y coordinate values. There's also the slightly strange fact that you have to wrap a matrix of points in a list to make a polygon. So because of the row-column thing, there's a tendency to do
...
and the first and last rows don't match.
...
### `st_snap`
Another possible fix for the floating point issue is snapping points to the coordinates of another object before applying operations. So this works, although it is not as clean as the `st_precision` option. On the other hand, it does work on plain geometry objects, not only on those that have bundled up into collections.
