...
```{r}
diffs <- function(v1, v2) {                                         # <1>
  outer(v1, v2, "-")
}

ll_distances <- function(p1, p2) {
  dlon <- diffs(p1[, 1], p2[, 1])
  dlat <- diffs(p1[, 2], p2[, 2])
  asin(sqrt(
    sin(dlat / 2) ^ 2 +
    outer(cos(p1[, 2]), cos(p2[, 2]), "*") * sin(dlon / 2) ^ 2      # <2>
  ))
}
...
spaced_points <- function(n = 50, choice_scaling = 1.5,
  ...
  points <- c(runif(1, input_bb[1], input_bb[3]), 
    ...
    matrix(ncol = 2)
  for (i in 1:(n-1)) {
    n_candidates <- ceiling(log(i * exp(1) * choice_scaling))       # <3>
...
```
1. Use the `outer` function to do all pairwise differences between two vectors.
2. Again use `outer` to do all pairwise products of the cosines.
3. The `choice_scaling` parameter determines how rapidly the number of candidate points grows with the size of the existing data set. It should be strictly greater than 1 or no points will ever get added! Including a log factor stops the speed of the algorithm from falling too rapidly as points are added.
4. The `apply(1, min)` operation finds the smallest distance in each row (i.e. distance to nearest neighbour in the existing set of points), and `which.max()` identifies the row with the largest minimum distance.
