...
description: Some other approaches I found along the way
...
## Some preliminaries
To make things easier, I am going to generate patterns in the unit square, and then transform them to patterns on the globe. Before we get started here are all the libraries I'm using.
...
2. `spacefillr` provides additional quasi-random sequences.
3. `spatstat` provides a wide range of spatial point processes.
4. For replicability.
...
plot_pp <- function(df) {                             # <1>
  ggplot() +
    ...
    coord_equal(expand = FALSE) +                     # <2>
    theme(panel.background = element_rect(fill = NA)) # <3>
}

pattern1 <- tibble(x = runif(n_points),
                   y = runif(n_points),
...
### A couple of patterns from spatial point processes
There are a couple of possibilities here. 

First is a _stratified point process_ where we randomly place a single point in each of a grid of locations: a stratified random point pattern. This is why I picked a number of points that is an exact square so that I can subdivide the unit square evenly into that number of square cells.

```{r}
pattern2 <- rstrat(nx = sqrt(n_points), ny = sqrt(n_points), k = 1) |>
  as.data.frame() |>
  mutate(generator = "Stratified point process")
```

Next is the point process I started from, _sequential spatial inhibition_. I have to set an _inhibition distance_, and as a rough value choose the expected minimum nearest neighbour distance $1/2\sqrt{\lambda}$ where $\lambda$ is the pattern intensity. This process randomly places points but when a new point is placed closer to an existing point than the inhibition distance it tries again, until it gets the requested number of points.

```{r}
#| fig-cap: "Sequential spatial inhibition"
#| fig-width: 4
#| fig-height: 4
pattern3 <- rSSI(r = 1 / 2 / sqrt(n_points), n = n_points) |>
  as.data.frame() |>
  mutate(generator = "Sequential Spatial Inhibition (SSI)")
```

::: {#fig-two-spatial-point-process-patterns layout-ncol=2}
```{r}
#| fig-cap: "Stratified random point process"
#| echo: false
plot_pp(pattern2)
```
...
Two patterns produced by spatial point processes in `spatstat`
:::

Both these patterns appear clearly more evenly spaced with less 'clumping' than the example in @fig-uniform-random.

### Quasi-random sequences
These are the processes that were new to me, which showed up when I started looking for R packages that could do even sampling in 2D spaces. The package that showed up was [`spacefillr`](https://www.spacefillr.com/). This implements a number of such sequences, but we'll look here at the Halton sequence (because its workings are easy to understand) and the Sobol sequence (less easy to understand, but easy to run).
...
#### Sobol sequences
Another type of quasi-random sequence is the [:_Sobol sequence_](https://en.wikipedia.org/wiki/Sobol_sequence) which is provided by the `spacefillr::generate_sobol_set` function.

```{r}
pattern5 <-  generate_sobol_owen_set(n_points, 2) |> # <1>
  as.data.frame() |>
  rename(x = V1, y = V2) |>
  mutate(generator = "Sobol")
```
1. Second parameter is the desired dimensionality.
...
Points generated by a Sobol sequence
:::

This appears more 'random' while preserving the limited clumping property of the Halton sequence.
...
] I have simplified matters by not calculating Euclidean distances and the results appear reasonable for my present very limited purpose. It is unclear to me if I needed to calculate toroidal distances but I have retained that feature of the algorithm.
...
spaced_points <- function(n = 50, choice_scaling = 1.5) {
  ...
  points |> 
    as.data.frame() |>
    rename(x = V1, y = V2) |>
    mutate(generator = "Blue noise")
}
```
1. `outer` applies subtraction to all pairwise combinations of values in the supplied vectors.
2. Choosing the smaller of the differences in coordinates between `x` and `1-x` and  is what makes the operation toroidal.
3. Manhattan distance is simpler than either squared or Euclidean distance, and appears sufficient.
4. The `choice_scaling` parameter determines how rapidly the number of candidate points grows with the size of the existing data set. It should be strictly greater than 1 or no points will ever get added!
5. Including a log factor in this scaling stops the speed of the algorithm from falling too rapidly as points are added.
6. The `apply(1, min)` operation finds the smallest distance in each row (i.e. distance to nearest neighbour in the existing set of points), and `which.max()` identifies the row with the largest minimum distance.
7. This makes a random starting point.
...
#| fig-height: 4
#| echo: false
pattern6 <- spaced_points(n = n_points)
plot_pp(pattern6)
```

A pattern generated using a simple 'blue noise' algorithm based on picking the most remote random additional point among a set of choices
:::
...
transform_to_globe <- function(df, proj) {
  df |>
    mutate(x = x * 360 - 180, y = acos(y * 2 - 1) * 180 / pi - 90) |> # <1>
    st_as_sf(coords = c("x", "y"), crs = 4326) |> 
    st_transform(proj)  
}

all_points <- bind_rows(pattern1, pattern2, pattern3,
                        pattern4, pattern5, pattern6) |>
  transform_to_globe(projection) |>
  mutate(generator = ordered(generator, c("Uniform random", "Stratified point process",
                                          "Sequential Spatial Inhibition (SSI)", 
                                          "Halton", "Sobol", "Blue noise")))

ggplot(globe) +
  geom_sf(fill = "#cceeff", linewidth = 0) +
  geom_sf(data = world, fill = "grey", linewidth = 0) +
  geom_sf(data = all_points, size = 0.35, colour = "red") +
  facet_wrap( ~ generator, ncol = ifelse(aspect_ratio > 1, 2, 3)) + # <2>
  theme_void()
```
1. Convert to longitude-latitude from the unit square, applying the cosine correction to the y axis.
2. This `ifelse` clause makes for a nicer arrangement of small multiples if the maps are wider than they are high.

Side-by-side comparison of six patterns projected onto the globe
:::

With the obvious exception of the Uniform random pattern, which exhibits definite clumping, all of these seem to do a reasonable job of producing a random arrangement of points evenly distributed over the globe. The Halton and Sobol sequence generate patterns may have some (non-straight) linear 'clumps' although it's hard to know if this is a result of knowing that the generating processes involved can yield such alignments.
...
ggplot(xy) +
  ...
  scale_fill_manual(values = c4a("brewer.yl_gn_bu"),                # <1>
                    breaks = 1:6, guide = "legend", name = "#Points") +
  annotate(geom = "path", x = x, y = y, linewidth = 0.2) +
  coord_equal() +
  facet_wrap( ~ generator, ncol = ifelse(aspect_ratio > 1, 2, 3)) +
  theme_void()
```
...
Side-by-side comparison of hexbin density plots of the six point patterns
:::

The uniform random pattern clearly exhibits the most uneven distribution of points. Somewhat to my surprise my 'homebrew' blue noise compares well with the other four patterns for evenness of distribution. This is particularly nice given that it requires no spatial parameter to be supplied, only one that (probably marginally) affects the quality of the patterns produced.

It is intriguing to me that this is seemingly not a standard spatial point process.
