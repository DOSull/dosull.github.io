...
With pretty high accuracy, if a place is in the 'north' island, it is in the 'east' island! There are probably better ways to 'put a number' on this observation (contingency tables and whatnot), but a correlation coefficient will do for now.
...
voronoi_islands <- nz |>
  ...
  summarise()
```
1. The raw Voronoi polygons produced by `st_voronoi` extend well beyond the area of interest, so make a bounding box to clip them.
