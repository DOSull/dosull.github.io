...
A little way into the process, I realised that since I did last year's [30 Day Map Challenge](https://30daymapchallenge.com/) (you can see my efforts [here](https://dosull.github.io/30-day-maps-2023/)) I use `ggplot2` a lot more for my everyday mapping needs than I do `tmap`. That's in spite of having taught `tmap` for several years, a choice I made because its learning curve is less steep than `ggplot2`'s. Anyway, long story short, migrating the code on my book website from `tmap` 3 to 4 turned into more of a mixed bag: migrating some code to `tmap` 4, and some to `ggplot2`.
...
To illustrate, here's an old stand-by dataset I often use: TB cases in Auckland City in 2006 by Census Area Unit (remember those?):
...
Note that I am using `cols4all` scales here, because this is the preferred colour palettes package for `tmap`, and it provides a wide array of options. For casual mapping, we likely don't care about the grid, and we can get rid of that too using `theme_void()`:
...
```
Nice! Note that the same `tm_scale_intervals()` function is used here to specify colours in this case as in the choropleth map case. We can also have a continuous colour ramp, and layer on top a hillshade. First make the hillshade using some `terra` functions:

```{r}
slope <- dem |> terra::terrain("slope", 
                               unit = "radians")
aspect <- dem |> terra::terrain("aspect", 
                                unit = "radians")
...
tm_shape(dem) + 
  ...
  tm_layout(legend.frame = FALSE, legend.show = FALSE)
```
...
Anyway, while this approach is fine for a small raster, even here using the `x-y` dataframe equivalent to the raster has 35,000 or so rows, so this doesn't scale well. 
