...
&bull;&nbsp;the default colour ramp is black-to-blue, which might be preferable to the default muddy browns we've become accustomed to, but is an unusual choice for a map.
:::
...
ggplot() +
  ...
  scale_fill_continuous_c4a_seq(
    palette = "brewer.reds") + # <1> 
  coord_sf(datum = st_crs(ak)) # <2>
```
1. Use palette Brewer 'Reds'
2. Use the coordinate system of the data for the grid
...
m2 <- tm_shape(ak) +
  tm_polygons(
    ...
    fill.scale = tm_scale_intervals(
      values = "brewer.reds", style = "equal", n = 6)) +    # <1>
  tm_layout(
    title = "Equal intervals", title.size = 0.8,            # <2>
    frame = FALSE, legend.frame = FALSE) 
...
m3 <- tm_shape(ak) +
  tm_polygons(
    fill = "TB_RATE",
    fill.scale = tm_scale_intervals(
      values = "brewer.reds", style = "quantile", n = 6)) + # <3>
  tm_layout(
...
m4 <- tm_shape(ak) +
  tm_polygons(
    fill = "TB_RATE",
    fill.scale = tm_scale_intervals(
      values = "brewer.reds", style = "sd")) +              # <4>
  tm_layout(title = "Std. Dev.", title.size = 0.8,
            frame = FALSE, legend.frame = FALSE)

tmap_arrange(m1, m2, m3, m4, ncol = 2)
```
1. style = "equal" to get equal intervals
2. add a title to show the classification style
3. style = "quantile" for quantiles
4. style = "sd" for standard deviation breaks
...
```
1. tm_raster 'announces' that the data are raster so that some visual variables might be interpreted differently, e.g. col is now the 'fill' whereas on a polygon layer it is the outline colour
...
hillshade <- shade(slope, aspect, angle = 35, direction = 135)
```
...
```
1. Note that for ggplot the colours are still a 'fill' here
...
&bull;&nbsp;Where I previously used web maps in `tmap` to get an overview of my data, now I tend to use QGIS. If I _really_ need a web map, I will certainly use `tmap`; and

&bull;&nbsp;I am not a fan of map junk (hence: junk). Don't get me wrong, I'll but a north arrow and scalebar on a map [if necessary](https://www.esri.com/arcgis-blog/products/arcgis-pro/mapping/put-a-north-arrow-on-it/), I just don't make many maps like that.
:::
