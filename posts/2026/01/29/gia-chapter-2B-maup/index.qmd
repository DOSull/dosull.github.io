...
lightbox:
  match: auto
aliases: 
  - "/posts/2026-01-29-gia-chapter-2B-maup/index.html"
code-fold: show
...
And here's how the correlations among 50 sets of data change when the 32&times;32 grids are aggregated to 32 areas.
...
The problem with this perspective is that observing a correlation of (say) 0.5 in aggregated data doesn't tell us very much about how strong the correlation might be in the more disaggregate underlying data. Based on the admittedly one-off result above, a post-aggregation correlation (`c1`) between two attributes in the range 0.475 to 0.525 might arise from disaggregate data with a correlation (`c0`) between those attributes anywhere between -0.031 and 0.466:

```{r}
#| label: range-for-c1-around-0.5
df_by_range |> 
  filter(c1 > 0.475, c1 <= 0.525) |>
  pull(c0) |>
  summary()
```
...
To explore this we can apply 50 different regionalisations to the data and see if we can make any sense of the results. I made these previously using the `random_regionalisation()` function, so we read them in from a file.^[This just makes it a bit easier to manage variability from run to run as I edit and re-edit this post...]
...
#| fig-cap: Each initial correlation as a vertical line from the lowest to highest correlation after regionalisation, with the means overplotted as points.
#| fig-width: 5
#| fig-height: 6
#| code-fold: true
...
At any rate, it's a problem if you are working with aggregated data.^[And we haven't even looked into whether constructing zones for homogeneity on one variable affects outcomes for other variables more or less than these random zonings...] 
...
Available at http://www.qmrg.org.uk/catmog.] it's more a pragmatic response to the nature of spatial data.
