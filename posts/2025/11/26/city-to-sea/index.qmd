...
We can make a profile by sampling at equally spaced intervals along the linestring using`st_line_interpolate` and `terra::extract`. The walkway is about 14.5 km, so 3000 steps is roughly every 5m. That seems about right when the elevation data is at 1m resolution.
...
named_points <- named_segments |>
    mutate(
        geom = st_line_interpolate(geom, 0.5, normalized = TRUE),
...
g_map <- ggplot() +
    ...
    guides(fill = "none") +
    geom_path(
        data = city_to_sea_yx, aes(x = x, y = y),
        linetype = "dashed"
    ) +
    geom_text(
...
route <- shortest_paths(
    G, start_finish[1], start_finish[2]
)$vpath |>
    unlist()
```
...
ggplot() +
    geom_path(
        data = city_to_sea_2 |> st_coordinates() |> data.frame(),
        aes(y = X, x = -Y)
    ) +
    coord_equal() +
    theme_void()
