...
We now apply a transformation from the lat-lon Cartesian coordinates to the globe that is equal-area as shown in the [previous post](../2021/10/20/random-even-points-on-the-globe/index.qmd). We can combine all the points into a single data set and use `facet_wrap` for side-by-side comparison.
