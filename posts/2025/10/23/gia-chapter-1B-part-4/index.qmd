...
lightbox:
  match: auto
aliases: 
  - "/posts/2025-10-23-gia-chapter-1B-part-4/index.html"
...
peaks <- dem |>
  ...
  raster_to_points()
```
1. `\(x)` defines an anonymous function of the 3x3 focal matrix `x`.
...
#| fig-cap: Geomorphons detected by whitebox tools with peaks (point-up triangles) and pits (point-down triangles)
#| fig-width: 8
#| fig-height: 11
#| code-fold: true
...
[Jo Wood's](https://www.gicentre.net/jwo/index) impressive [Landserf](https://www.staff.city.ac.uk/~jwo/landserf/) software, which still runs in spite of the most recent published manual being dated 2009, can extract surface networks. Landserf is fun to explore, although its interface takes some getting used to.^[While Jo's morphometric work remains impressive (I might use Landserf for one of the remaining posts in this series...) he has long since switched focus to visualization methods, and [that work is well worth checking out](https://observablehq.com/@jwolondon).]

The most recent work I've found on this topic is by [Eric Guilbert](https://scholar.google.com/citations?user=QQqIxvAAAAAJ&hl=en) and colleagues (Guilbert 2021^[Guilbert E. 2021. [Surface network extraction from high resolution digital terrain models](https://dx.doi.org/10.5311/JOSIS.2021.22.681). _Journal of Spatial Information Science_ **22** 33-59], Guilbert et al. 2023^[Guilbert E, F Lessard, N Perreault, and S Jutras. 2023. [Surface network and drainagenetwork: towards a common datastructure](https://dx.doi.org/10.5311/JOSIS.2023.26.240) _Journal of Spatial Information Science_ **26** 53-77]) and hallelujah! there is runnable code that produces well-formed surface networks. 
...
and it's tricky to detect peaks, pits, and passes reliably due to the approximate nature of raster surface representations. In fact, comparing my results with Guilbert et al.'s my code's naïve approach identifies too few pits and peaks, and (surprisingly) too many passes. Some of my problems are my old friend floating point numbers and the question of when is a really small number actually zero. The Guilbert et al. papers linked in the footnotes go into some detail on how these issues can be resolved, but in essence it revolves around introducing the right number of diagonal connections between cells in the grid to maximise the number of saddle points detected, and satisfy the topological constraints.
