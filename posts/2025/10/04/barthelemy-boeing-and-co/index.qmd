...
lightbox:
  match: auto
aliases: 
  - "/posts/2025-10-04-barthelemy-boeing-and-co/index.html"
...
collapse_all_chains <- function(G) {      # <7>
  while (degree_distribution(G)[3] > 0) { # <8>
    G <- G |> collapse_chains()
  }
  G
}
...
8. Item 3 in the vector returned by `degree_distribution` is the number of degree-2 vertices in the graph.

### A nice plotting function
The `plot.igraph` function is difficult to control particularly when it comes to imposing equal aspect ratios and layering graphs in the same space on top of one another, so the somewhat _ad hoc_ function below converts graphs to `sfnetwork` graphs which can be plotted by `ggplot`.
