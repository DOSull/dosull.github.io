...
lightbox:
  match: auto
aliases: 
  - "/posts/2025-06-13-population-quadrants/index.html"
...
get_ggline <- function(straight_line, ...) { # <5>
    if (straight_line$B == 0) { # <6>
        ...
        geom_abline(aes(
            intercept = get_intercept(straight_line),
...
get_cells_one_side_of_line <- function(pts, sl, above = TRUE) { # <1>
    if (above) {
        pts |>
            mutate(chosen = sl$A * x + sl$B * y + sl$C > 0) |>
            pull(chosen)
    } else {
        pts |>
            mutate(chosen = sl$A * x + sl$B * y + sl$C <= 0) |>
...
get_quadrant_pops <- function(pts, sl1, sl2) {
    ...
    c(
        sum(pts$pop_250m_grid[above1 & above2]), # <2>
...
plot_range <- function(angles) {
    ...
    ggplot(df |> select(angle, pop1:pop4) |>
        ...
        scale_x_continuous(
            breaks = angles[seq(1, length(angles), 10)],
            minor_breaks = angles
        ) +
