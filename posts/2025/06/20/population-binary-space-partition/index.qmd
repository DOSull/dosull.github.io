...
lightbox:
  match: auto
aliases: 
  - "/posts/2025-06-20-population-binary-space-partition/index.html"
...
In a [recent post](/posts/2025/06/13/population-quadrants/) I developed some R code to quadrisect the population of Aotearoa New Zealand. At the end of that post I airily suggested
...
Central to this whole exercise is a function that given a dataframe of x, y, and population data, along with a bounding extent, returns two rectangles within the extent, each containing half the population. This is accomplished using a weighted median as in the [previous post](/posts/2025/06/13/population-quadrants/). Because we are working only with north-south or east-west subdivisions there isn't any especially complicated geometry needed to handle this.
...
split_population <- function(pop_pts, bounds) {
    ...
    bbs <- list()
    if (height > width) { # cut north-south at median y          # <3>
        my <- weighted.median(xy$y, xy$pop_250m_grid)
        south <- xy |> filter(y <= my)
        x1 <- min(south$x) - 125
        x2 <- max(south$x) + 125 # <4>
        y1 <- min(south$y) - 125
        y2 <- my
    ...
    } else { # cut east-west at median x
...
```
1. Unpack the bounding box to west, east, south, and north values, and use these to get width `w` and height `h`. 
...
ggplot(nz) +
    geom_sf(fill = "grey", lwd = 0) +
    geom_sf(
        data = all_levels |> filter(level > 1),
...
ggplot() +
    ...
    geom_sf(
        data = all_levels |> filter(level == 11),
...
tm_shape(top_level) +
    tm_polygons(
        fill = "#ff101040", border.col = "white",
