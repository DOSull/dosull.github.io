...
Now that we have a single linestring, we need also elevation data. This is extracted from Wellington LiDAR imagery available [here](https://data.linz.govt.nz/layer/105023-wellington-city-lidar-1m-dem-2019-2020/).
...
And now we can make a plot of elevation by distance along the walk. It's instructive here to show the elevation and distance at the same scale, and also with the elevation exaggerated eight-fold. Clearly, this walk is not as hilly as it feels!
...
And now, as before we find the elevation of each of these points. I also make a simple dataframe version of the data so we can plot the data based on the distance and elevation rather than the planimetrix map coordinates.
...
I explored doing this with an actual map projection or by faking a projection using matrix multiplication, but in the end decided it was easier just convert my data to plain dataframes with `x` and `y` attributes and alter those as needed.
...
A more annoying issue is that because the labels are placed differently in each representation the `check_overlap` filter in each plot has chosen a different set of labels to include. A better approach might would likely be to hand pick the labels to include, and ensure that all of the selected labels appear in both plots. That wouldn't be too hard to arrange, but I'm happy with this version of the map and will leave it as is.

## Addendum: making a linestring from messier data
Above I made the unified linestring representation of the City to Sea Walkway from an input file containing a single multilinestring file. The version of the data with named segments on the walk turns out to be less cooperative than that.
...
If the collection of linestrings formed a single linear chain, we'd expect there to be no linestrings that touch three others, and only two (one at the start, and one at the end) that touched only one other. We can map this (although you'll have to squint to see the 'spurs' or 'on-ramp' segments, that are causing the difficuly&mdash;click on the map for a closer look).
...
To piece together a maximal single linear route in this case we can proceeds as follows. First, we determine which segments in the route touch, and use those relations to build a graph.
...
Now, we can find the longest shortest path in the graph (in graph terms, its [_diameter_](https://en.wikipedia.org/wiki/Diameter_(graph_theory))). This connects the most remote ends of the graph, which we assume will give us appropriate start and end points for the whole of the City to Sea Walkway.
...
#| fig-cap: A quick and dirty map of the route.
#| fig-width: 12
#| fig-height: 2
#| code-fold: true
ggplot() +
