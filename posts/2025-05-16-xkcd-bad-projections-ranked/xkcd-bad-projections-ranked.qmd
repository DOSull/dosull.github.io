---
title: "XKCD bad projections: ranked!"
subtitle: "And reverse-engineered"
from: markdown+emoji
...
[XKCD 2999](https://xkcd.com/2999) just doesn't work for me. I appreciate the attempt at many to one projection, that is to say more than one place on Earth mapping onto a single location in the map. It's certainly a relief to know that Melbourne is preserved.^[I have relatives there.] 

I didn't even try to replicate this one as it seems self-evidently something you'd have to do in a drawing package. Overall, I think the idea of two places into one is much better realised at [#2 in this list](#bad-map-projection-abslongitude).
...
The whole Greenland thing in map projections is a bit played out. I realise it continues to blow minds how exaggerated its size is in the Mercator projection.^[Maybe someone should have a word with a recently elected president on this matter.] Anyway, it's not too hard to remake [XKCD 2489](https://xkcd.com/2489) using R, provided you aren't too respectful of, you know, actual projections.
...
2. After rescaling we have to tell a white lie and tell the data it is Mercator projected.
...
Armed with the ludicrously precise lat-lon coordinates^[See [XKCD 2170](https://xkcd.com/2170) on that subject] Google provided me for this spot, I figure out how to approximate this map:
...
madagascator <- World |>
  ...
  st_set_crs("+proj=laea +lon_0=150 +lat_0=90") |>               # <1>
  st_transform("+proj=merc")                                     # <2>
...
1. Tell `sf` that the data are centred on somewhere different than they really are! Some experimentation was required here to find a central 'meridian' that didn't cut Antarctica in half.
2. Apply Mercator to the recentred data. There's probably a setting of the Oblique Mercator (`+proj=omerc`) that can do this sequence of transforms in one go, but since what I have works, I am leaving well alone.
...
```{r}
#| code-fold: true
ctr <- c(-98.583333, 39.833333)
anti <- c(ctr[1] + 180, -ctr[2])                                   # <1>
proj1 <- str_glue("+proj=aeqd +lon_0={ctr[1]} +lat_0={ctr[2]}")    # <2>
proj2 <- str_glue("+proj=aeqd +lon_0={anti[1]} +lat_0={anti[2]}")  # <3>
...
states_big <- usmap::us_map() |>
  ...
  st_transform(proj1) |>
  mutate(geometry = geometry * matrix(c(5, 0, 0, 5), 2, 2)) |>     # <4>
  st_set_crs(proj1)                                                # <5>

ggplot(states_big) + geom_sf() + theme_minimal()
```
1. The antipode of the central location.
2. Forward projection: this can likely be any sensible projection with a central coordinate pair.
3. The distance projection from the antipode, that will 'invert' the space.
4. This multiplication expands the extent of the states.
5. After the multiplication we have to reset the projection.
...
Personally, living in Aotearoa New Zealand I enjoy the genre of 'upside down' maps, but have always felt that the notion they change your whole perspective on the world is overstated, so [XKCD 1500](https://xkcd.com/1500) is a nice gentle undermining of that, which I appreciate.^[I'm not sure how I feel about 'The United Kingdom of Great Britain and Southern Ireland', but that's a whole other matter...]
...
5. Combine the hemispheres shift them so they are centred on the prime meridian and transform to Equal Earth
...
That's a wrap (or fold, or cut, or... some transformation anyway).