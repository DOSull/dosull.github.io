...
2. Group on the `route` attribute and summarise to create multilinestrings.
...
Among other reasons, actually existing routes are different from all possible ones because the [longest scheduled flights max out at a bit over 15,000km](https://en.wikipedia.org/wiki/Longest_flights#Longest_passenger_flights)^[I regret to say I've been on a couple of these, a side-effect of being from Ireland and living in New Zealand. I can't recommend such super-long flights in economy unless it's in an [A380](https://en.wikipedia.org/wiki/Airbus_A380). But can I just say? Those things are _awesome_.]

Sadly, it's difficult to source open data on regularly scheduled flights, because it's commercially valuable information. The best I could do was from [openflights.org](https://openflights.org/data.php), but as they say "The third-party that OpenFlights uses for route data ceased providing updates in June 2014".^[Oh well... this is a fun little project but not one worth spending any money on.] Anyway, we can filter out routes from our off-by-one dataset that don't appear in this list.
