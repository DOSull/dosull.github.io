...
I'll explain using my goto tool for general data wrangling, R's [`sf` package](https://r-spatial.github.io/sf/).
...
nz_r |> 
  writeRaster("nz.gpkg", 
              gdal = c("RASTER_TABLE=nzr", "APPEND_SUBDATASET=YES"))
```
...
Of course not. There's an argument to be made that every format that perpertuates the simple features paradigm is a bad as every other. I even wrote [a book](https://computinggeographically.org) that is &mdash; kind of &mdash; all about this. It's one of the mysteries of the evolution of geospatial that topology was embedded in the 'standard' formats, until it wasn't. For what it's worth, I think we have relational databases to blame for that.

GeoPackages don't get us out of [floating point geometry hell](../2021-12-08-low-level-handling-sf-objects/) either.
