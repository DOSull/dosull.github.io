...
As we might expect, a single layer with the same layer name as the file itself. Say we buffer our data and want to store it back into the same file, then we can do the below, as long as we provide a new layer name to store it in:
...
Meanwhile, if you open this two-layer GPKG in QGIS you'll see this:
